...
{
	printk(KERN_ERR "Alan-IMX219_NAME: [%s]\n", IMX219_NAME);
	printk(KERN_ERR "Alan-sensor->module_name: [%s]\n", sensor->module_name);
	printk(KERN_ERR "Alan-sensor->len_name,: [%s]\n", sensor->len_name);
...
static long imx219_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	printk(KERN_ERR "Alan-imx219_ioctl: cmd:%d\n", cmd);
	switch (cmd) {
	case RKMODULE_GET_MODULE_INFO:
		printk(KERN_ERR "Alan-imx219_ioctl: RKMODULE_GET_MODULE_INFO\n");
